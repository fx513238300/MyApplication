...
public class RateListActivity extends ListActivity implements Runnable{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //setContentView(R.layout.activity_rate_list);父类中已有布局不需要再自己设置
    ...
    public void run() {
        //获取网络数据，放入list带回主线程中
        List<String>retList=new ArrayList<String>();   //用于主线程
        ...
        if(curDateStr.equals(logDate)) {
            ...
            try {
                ...
                //android:usesCleartextTraffic="true"传输协议
                Log.i(TAG, "run:" + doc.title());
                Elements tables = doc.getElementsByTag("table");
                Element table2 = tables.get(1);//定位table
                //获取TD中的数据
                Elements tds = table2.getElementsByTag("td");
                List<RateItem> rateList = new ArrayList<RateItem>();  //用于写入数据库
